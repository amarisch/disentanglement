...
# python3 generate_data.py --folder patdata48_all5_seq10_feat65 --los 48 --seq_num 10
# python3 generate_data.py --folder patdata24_all5_seq10_feat65 --los 24 --seq_num 10
# python3 generate_data.py --folder patdata24_all5_seq20_feat65_ff --los 24 --seq_num 20 --ff
...
def parse_simple(mimic_id, icu_48, df2, fs, mean, std, topfeats, los=48):
#     print("mimic id: {}".format(mimic_id))
    #DATA_MIN_HOURS = 30 
    # patient record must have 60% non-all zero rows
    DATA_MIN_HOURS = int(0.6*los)
    ...
    evals = pat_new[topfeats].values.tolist()
    evals = (np.array(evals) - mean) / std
    forwards = pd.DataFrame(evals).fillna(method='ffill').values.tolist()[:los]
    # get ICD code for this entry
    label = icu_48.loc[icu_48['MIMIC_ID'] == mimic_id,'ICD_LABEL'].values[0]
    rec = {'label': label}
    rec['forward'] = forwards
    rec = json.dumps(rec)
    fs.write(rec + '\n')
    return rec
...
def get_icu48df(multilabel=1, los=48, seq_num=10):
    ...
    df_pivot = df_pivot[['SUBJECT_ID','HADM_ID','heart','lungs','liver','gi','kidney','type']]
...
#     icd_df = df_pivot.loc[(df_pivot.iloc[:,2:].sum(axis=1) == 1)]
    ...
    patdf['type'] = col
    patdf['ICD_LABEL'] = patdf.iloc[:,2:7].values.tolist()
...
def parse_arguments():
    ...
    parser.add_argument('--los', type=int, default=24)
        ...
        for file in patfiles:
            ...
            for mimic_id in data['MIMIC_ID'].unique():
                if mimic_id in icu_48['MIMIC_ID'].tolist():
                    rec = parse_simple(mimic_id, icu_48, data, fs, mean, std, topfeats, los)
                if mimic_id in icu_48_2['MIMIC_ID'].tolist():
                    rec = parse_simple(mimic_id, icu_48_2, data, fs, mean, std, topfeats, los)
                if mimic_id in icu_48_3['MIMIC_ID'].tolist():
                    rec = parse_simple(mimic_id, icu_48_3, data, fs, mean, std, topfeats, los)
                if mimic_id in icu_48_4['MIMIC_ID'].tolist():
                    rec = parse_simple(mimic_id, icu_48_4, data, fs, mean, std, topfeats, los)
                if mimic_id in icu_48_5['MIMIC_ID'].tolist():
                    rec = parse_simple(mimic_id, icu_48_5, data, fs, mean, std, topfeats, los)
